...
export function buildExtensionLoginRedirectUrl(requestUrl: URL) {
  const loginUrl = new URL(requestUrl.toString());
  const currentStartUrl = `${requestUrl.pathname}${requestUrl.search}`;
...
export async function createExtensionExchangeCode({
  ...
  const { error } = await supabaseClient.from("extension_auth_codes").insert({
    code_hash: codeHash,
    user_id: userId,
    email,
...
export async function createExtensionSessionForConsumedCode(
  ...
  const { data: linkData, error: linkError } = await clients.admin.auth.admin.generateLink({
    type: "magiclink",
    email: code.email,
  });
  ...
  const { data, error } = await clients.auth.auth.verifyOtp({
